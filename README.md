...
카드마을 `<CardVille>` 1.0.17은 첫 접속 때 보이던 “카드마을을 여는 중” 부팅 문구를 제거하고, 바로 시작 화면처럼 보이도록 바꾼 UX 안정화 버전입니다. 인트로 영상은 이제 앱을 여는 순간이 아니라 사용자가 **게임 시작**을 누른 뒤 게임 리소스를 준비하는 동안 재생됩니다.
...
1.0.17은 첫 화면 진입 속도를 위해 대형 카드 이미지 5000장을 한 번에 강제 연결하지 않습니다. 게임 시작을 누른 뒤 필요한 UI/카드팩/효과 리소스를 단계적으로 준비합니다.
