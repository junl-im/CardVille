## 1.0.80 - LobbyComfortCleanDelta / DocsHygieneDeltaPatch

- 현재 기준 버전은 1.0.80입니다.
- 이번 패스는 광장 빈 공간 터치 이동의 속도/대각선 추적/NPC 오터치 재발을 줄이는 안정화입니다.
- `floor-walk-comfort-timing-v180`: 광장 자유 이동은 920~2400ms 범위의 거리 기반 보행으로 느려졌습니다. 이전 `duration: 430`, `naturalSpeed = 145` 경로는 금지합니다.
- `floor-walk-trail-follow-v180`: 고양이는 `Math.hypot(dx, dy)` 기반으로 이동 벡터 뒤를 따라가므로 대각선 이동에서도 옆으로 튀지 않습니다.
- `npc-clear-building-lane-v180`: NPC는 건물 이름표/건물 터치 영역 앞을 피하도록 좌우 가장자리 보행 동선으로 재배치했고, 터치 박스를 더 작게 유지합니다.
- `tools/check-lobby-comfort-v180.mjs`가 광장 이동 속도, 대각선 추적, NPC 좌표, 문서 위생, 로딩바/진행바 재발을 함께 검수합니다.
- 1.0.80 - DocsHygieneDeltaPatch 정책도 유지합니다.
- 패치 정보는 `README.md`와 `AI_HANDOFF_CARDVILLE.md`에만 남깁니다. 새 docs 문서와 `PATCH_NOTES*`, `CHANGELOG*`, `RELEASE_NOTES*`, 임시 txt 기록 파일은 금지합니다. README.md와 AI_HANDOFF_CARDVILLE.md 외 기록 파일 금지.
- Delta Patch는 v1.0.79 통파일 기준 변경 파일만 담는 덮어쓰기용이며 `public/assets` 전체는 포함하지 않습니다.
...
### 1.0.80 PlazaWalkNpcPolish / 자연스러운 대각선 보행 / NPC 오터치 분리

- 목적: 사용자가 지적한 “광장 이동이 너무 빠르고 부자연스럽다”, “대각선 이동도 가능하면 좋겠다”, “NPC가 건물 이름/터치 앞을 가린다” 문제를 해결합니다.
- `floor-walk-natural-pace-v178`: 빈 공간 이동을 거리 기반 760~1680ms 보행으로 바꿨습니다.
- `floor-walk-diagonal-v178`: x/y를 동시에 보간하여 대각선 이동과 발자국 방향을 자연스럽게 처리합니다.
- `npc-building-touch-separation-v178`: NPC 좌표를 건물 이름표와 건물 터치 박스 옆으로 이동했습니다.
- `npc-tap-zone-shrink-v178`: NPC 터치 박스는 몸통 중심으로 축소했고 `npc-touch:*` 이름을 명시해 바닥 터치/건물 터치와 충돌 감지가 가능하게 했습니다.
- 패치 기준: `CardVille_v1.0.80_RawOffscreenCanvasGuard_Full.zip`에 `CardVille_v1.0.80_PlazaWalkNpcPolish_DeltaPatch.zip`을 덮어쓰는 방식입니다.
- 산출물 이름: `CardVille_v1.0.80_PlazaWalkNpcPolish_Full.zip`, `CardVille_v1.0.80_PlazaWalkNpcPolish_DeltaPatch.zip`.

### 1.0.80 RawOffscreenCanvasGuard / 덮어쓰기용 작은 패치 정책 / canvas-only 인트로 보강

- 목적: 사용자가 지적한 “패치 파일이 통파일 같다” 문제를 해결하기 위해, v1.0.80도 직전 통파일 기준 변경 파일만 담는 Delta Patch를 산출합니다.
- 패치 기준: `CardVille_v1.0.76_DeltaPatchCanvasGuard_Full.zip`에 `CardVille_v1.0.80_RawOffscreenCanvasGuard_DeltaPatch.zip`을 그대로 덮어쓰는 방식입니다.
- 핵심 수정: `delta-patch-overwrite-v176`, `raw-video-offscreen-v177`, `canvas-frame-preroll-v177`로 패치 제작/검증 기록을 추가했습니다. clean install은 Full ZIP, 업데이트는 Delta Patch ZIP을 사용합니다.
- 핵심 수정: `intro-canvas-only-video-surface-v176`와 `raw-video-offscreen-v177`로 raw video를 더 강하게 offscreen 처리하고, 실제 표시 표면은 canvas만 유지합니다.
- 핵심 수정: `floor-walk-interaction-v176`, `floor-walk-tap-debounce-v177`로 빈 광장 터치 이동이 설정/하단 안내/건물/NPC 터치와 겹치지 않도록 guard를 넓혔습니다.
- 추가 검증: `tools/check-delta-patch-v177.mjs`, `tools/check-canvas-hardening-v177.mjs`, `npm run check:delta-patch-v177`, `npm run check:canvas-hardening-v177`.
- 산출물 이름: `CardVille_v1.0.80_RawOffscreenCanvasGuard_Full.zip`, `CardVille_v1.0.80_RawOffscreenCanvasGuard_DeltaPatch.zip`.
- 검증 호환 앵커: CardVille_v1.0.75_CanvasIntroFloorPolish_Full.zip, CardVille_v1.0.80_DeltaPatchCanvasGuard_Full.zip, CardVille_v1.0.80_DeltaPatchCanvasGuard_DeltaPatch.zip.
- 이전 기록: 패치 ZIP이 통파일과 비슷한 용량인 이유는 v1.0.75까지 self-contained 패치가 public/assets 전체를 포함했기 때문입니다. v1.0.80부터 Delta Patch는 변경 없는 assets를 제외합니다.

- 검증 호환 앵커: IntroVideoHardVisible, IntroVideoMinFit, IntroNoOverlayPolish, IntroCleanSweep, LegacyGuardUIPolish, CanvasIntroFloorPolish, window.__CARDVILLE_INTRO_VIDEO_STARTED_AT__, MIN_INTRO_VIDEO_MS = 3000, input-watchdog-v166, 새 docs 문서 생성 금지, CardVille_v1.0.80_IntroVideoHardVisible_Full.zip, CardVille_v1.0.80_LegacyGuardUIPolish_Full.zip, CardVille_v1.0.80_IntroCleanSweep_Full.zip, CardVille_v1.0.80_IntroNoOverlayPolish_Full.zip.

### 1.0.80 CanvasIntroFloorPolish / native 영상 UI 우회 / 바닥 이동 hit guard 패스
...
- 산출물 이름: `CardVille_v1.0.80_CanvasIntroFloorPolish_Full.zip`, `CardVille_v1.0.80_CanvasIntroFloorPolish_Patch.zip`.


### 1.0.80 IntroMaskFloorMove / LegacyGuardUIPolish 호환 / 예전 코드 재발 방지 강화 패스
...
- 산출물 이름: `CardVille_v1.0.80_IntroMaskFloorMove_Full.zip`, `CardVille_v1.0.80_IntroMaskFloorMove_Patch.zip`.
- 이전 검증 호환 ZIP 앵커: CardVille_v1.0.80_LegacyGuardUIPolish_Full.zip, CardVille_v1.0.80_IntroCleanSweep_Full.zip, CardVille_v1.0.80_IntroCleanSweep_Patch.zip, CardVille_v1.0.80_IntroNoOverlayPolish_Full.zip, CardVille_v1.0.80_IntroGuardUIPolish_Full.zip, CardVille_v1.0.80_IntroVideoHardVisible_Full.zip.
...
- 추가 검증 호환 ZIP 앵커: CardVille_v1.0.80_IntroGuardUIPolish_Patch.zip, CardVille_v1.0.80_IntroVideoMinFit_Full.zip, CardVille_v1.0.80_CornerSweepStability_Full.zip, CardVille_v1.0.80_FlowFitUIPolish_Full.zip.
