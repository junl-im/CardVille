...
## 13.1. 1.0.33 스테이지/카드팩/앨범 폴리시 패스
...
1. 도서관 낱말 카드 플레이 화면을 CardVille 브랜드형 UI로 더 개편
2. 상점 허브의 오퍼 수량/쿨다운 표시/구매 결과 연출을 더 고급스럽게 확장
3. 연산 연구소와 기억의 숲 스테이지를 추가하고 난이도별 보상 차등 적용
4. 소년과 고양이 토큰을 실제 스프라이트 시트 애니메이션으로 교체
5. 계절 장식 시스템을 개별 오브젝트 에셋으로 추가
6. 실제 빌드 후 모바일 카카오 브라우저 터치/뒤로가기 테스트
7. 업로드 에셋팩의 이름 매핑과 WebP 생성 패치를 별도 진행
