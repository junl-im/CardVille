...
for (const token of ['cardville-back-overlay', 'makeOverlayButton', 'primeHistoryGuard', 'requestExit', 'data-cardville-back-overlay-v121']) {
...
if (!html.includes('data-cardville-no-prestart-cta')) throw new Error('index.html must explicitly mark no prestart CTA policy');
if (!intro.includes('data-cardville-hidden-until-playing')) throw new Error('Intro video must stay hidden until real playback starts');
...
console.log(`Security check passed. Version ${pkg.version}, local data hardening, auth validation and v121 back overlay, hidden video and no prestart CTA OK.`);
