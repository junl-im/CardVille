...
export const CARDVILLE_PANEL_CORNER_SWEEP_TAG = 'line-free-surface-v166' as const;
...
export function panel(scene: Phaser.Scene, x: number, y: number, w: number, h: number, radius = 28): Phaser.GameObjects.Graphics {
  ...
  w = box.width;
  h = box.height;
  const g = scene.add.graphics().setName(`${RESPONSIVE_SURFACE_SPREAD_TAG}:${CARDVILLE_PANEL_SEAM_FIX_TAG}:${CARDVILLE_PANEL_LINELESS_TAG}:${CARDVILLE_PANEL_CORNER_SWEEP_TAG}`);
