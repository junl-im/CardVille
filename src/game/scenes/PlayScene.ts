...
import { bodyText, cardText, mutedText } from '../ui/TextStyles';
...
export class PlayScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(195, 96, '같은 의미의 카드 2쌍을 맞춰보세요', bodyText(15)).setOrigin(0.5);
    ...
    this.add.text(195, 750, '뒤로가기: 스테이지 선택', mutedText(13)).setOrigin(0.5).setInteractive({ useHandCursor: true }).on('pointerup', () => this.scene.start('StageSelectScene', { modeId: this.modeId }));
  }

  private createCard(card: Card, x: number, y: number): void {
    const rect = this.add.rectangle(x, y, 126, 166, 0xffffff, 0.96).setStrokeStyle(4, 0x8fd3ff, 1).setInteractive({ useHandCursor: true });
    this.add.rectangle(x, y + 80, 112, 12, 0x000000, 0.08);
    const text = this.add.text(x, y, '?', cardText(30)).setOrigin(0.5);
  ...
  private flip(card: Card): void {
    ...
    card.text?.setText(card.label).setStyle(cardText(25));
    ...
    if (a.pair === b.pair) {
      ...
      this.time.delayedCall(520, () => {
        this.selected.forEach((card) => {
          card.flipped = false;
          card.rect?.setFillStyle(0xffffff, 0.96);
          card.text?.setText('?').setStyle(cardText(30));
        });
        this.selected = [];
      });
    }
